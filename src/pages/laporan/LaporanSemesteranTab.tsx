...
import AddLaporanSemesteranModal, {
  type LaporanSemesteranPayload,
} from "../../components/modals/AddLaporanSemesteranModal";
import AddRapatKinerjaModal, {
  type RapatKinerjaPayload,
} from "../../components/modals/AddRapatKinerjaModal";
...
export default function LaporanSemesteranTab() {
  const {
  ...
  // Modal states for Laporan Semesteran
  ...
  // Modal states for Rapat Kinerja
  ...
  // Save handlers for Laporan Semesteran
  const saveLaporan = useCallback(
    (data: LaporanSemesteranPayload) => {
      if (editingLaporanIndex !== null) {
        setLaporanSemesteranData((prev) =>
          prev.map((item, idx) => (idx === editingLaporanIndex ? data : item))
        );
        setEditingLaporanIndex(null);
      } else {
        setLaporanSemesteranData((prev) => [...prev, data]);
      }
      setOpenLaporanModal(false);
      setSavedTitle("Laporan Semesteran Tersimpan");
      setSavedOpen(true);
    },
    [editingLaporanIndex, setLaporanSemesteranData]
  );

  // Delete handler for Laporan Semesteran
  ...
  }, [deleteLaporanIndex, setLaporanSemesteranData]);

  // Save handlers for Rapat Kinerja
  const saveRapat = useCallback(
    (data: RapatKinerjaPayload) => {
      if (editingRapatIndex !== null) {
        setRapatKinerjaData((prev) =>
          prev.map((item, idx) => (idx === editingRapatIndex ? data : item))
        );
        setEditingRapatIndex(null);
      } else {
        setRapatKinerjaData((prev) => [...prev, data]);
      }
      setOpenRapatModal(false);
      setSavedTitle("Rapat Kinerja Tersimpan");
      setSavedOpen(true);
    },
    [editingRapatIndex, setRapatKinerjaData]
  );

  // Delete handler for Rapat Kinerja
  ...
  }, [deleteRapatIndex, setRapatKinerjaData]);
  ...
  // Table data for Laporan Semesteran
  const laporanTableData = laporanSemesteranData.map((item, idx) => ({
    ...item,
    _index: idx,
  }));

  // Table columns for Laporan Semesteran
  ...
  // Table data for Rapat Kinerja
  const rapatTableData = rapatKinerjaData.map((item, idx) => ({
    ...item,
    _index: idx,
  }));

  // Table columns for Rapat Kinerja
  ...
  return (
    <>
      <div className="min-h-[600px] p-6 space-y-8">
        ...
        <div>
          ...
          <div className="flex justify-end">
            ...
            >
              Tambah Laporan Semesteran
            </Button>
          </div>
        </div>

        {/* Section 2: Rapat Kinerja BUM Desa */}
        <div>
          <h2 className="text-lg font-semibold text-neutral-900 mb-4">
            Rapat Kinerja BUM Desa
          </h2>
          <div className="mb-4">
            <Table
              data={rapatTableData}
      ...
      {/* Modals for Laporan Semesteran */}
      <AddLaporanSemesteranModal
        ...
        title={
          editingLaporanIndex !== null
            ? "Edit Laporan Semesteran"
            : "Tambah Laporan Semesteran"
        }
        initialData={
          editingLaporanIndex !== null
            ? laporanSemesteranData[editingLaporanIndex]
            : undefined
        }
      />

      {/* Modals for Rapat Kinerja */}
      <AddRapatKinerjaModal
        open={openRapatModal}
        onClose={() => {
          setOpenRapatModal(false);
          setEditingRapatIndex(null);
        ...
        title={
          editingRapatIndex !== null
            ? "Edit Rapat Kinerja"
            : "Tambah Rapat Kinerja"
        }
        initialData={
          editingRapatIndex !== null
            ? rapatKinerjaData[editingRapatIndex]
            : undefined
        }
      />
      ...
      <ConfirmDialog
        ...
        title="Hapus Laporan Semesteran"
        message="Apakah Anda yakin ingin menghapus laporan semesteran ini? Tindakan ini tidak dapat dibatalkan."
      ...
      <ConfirmDialog
        ...
        title="Hapus Rapat Kinerja"
        message="Apakah Anda yakin ingin menghapus catatan rapat kinerja ini? Tindakan ini tidak dapat dibatalkan."
        confirmText="Ya, Hapus"
        cancelText="Batal"
        variant="danger"
      />
